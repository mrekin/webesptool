...
class APIService {
    ...
    private async downloadFileWithFilename(
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error(
                    `API Error (${response.status}): Failed to download firmware from ${url}`
                );
                ...
                switch (response.status) {
                    case 500:
                        userMessage =
                            'Server error occurred while preparing firmware file. This firmware version may not be available for download.';
                        break;
                    case 404:
                        userMessage =
                            'Firmware file not found. This version may not be available for this device.';
                        break;
                    case 429:
    ...
    private async downloadFile(endpoint: string, params: Record<string, string>): Promise<Blob> {
        const url = `${this.baseUrl}${endpoint}?${new URLSearchParams(params)}`;
    ...
    async getAvailableFirmwaresWithSource(source: string): Promise<AvailableFirmwares> {
        const params = source ? `?src=${encodeURIComponent(source)}` : '';
     ...
     */
    async getArchiveList(source: string): Promise<{ name: string; size: number }[]> {
        const params = new URLSearchParams({
            type: 'archives',
            repo: source
        });
     ...
     * Note: Archives are repository-level, not device-specific
     */
    async downloadArchive(source: string, filename: string): Promise<FirmwareDownloadResponse> {
        const params = {
            type: 'archives',
            repo: source,
            file: filename
