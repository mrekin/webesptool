<script lang="ts">
    ...
    function getDownloadOptions(
        ...
        mode: 'update' | 'full',
        locale: any,
        sourceType: RepositoryType | undefined
    ): DownloadOption[] {
        ...
        if (isESP32Device(deviceType as DeviceType)) {
            options.push({
                ...
                label:
                    mode === 'full'
                        ? $locales('downloadbuttons.full_flash_device')
                        : $locales('downloadbuttons.update_device'),
                mode: mode === 'full' ? '2' : '1', // 2=full, 1=update
                available: true,
                icon: '🔧',
                description:
                    mode === 'full'
                        ? $locales('downloadbuttons.complete_firmware_wipe')
                        : $locales('downloadbuttons.update_existing_firmware')
            });
        }
    ...
    > {
        ...
        for (const part of manifest.builds[0].parts) {
            try {
                ...
                throw new Error(
                    `Failed to download firmware part from ${part.path}: ${(error as any).message}`
                );
            }
        }
...
<h2 class="mb-6 flex items-center justify-between text-xl font-bold text-orange-200">
    ...
    <div class="space-y-4">
            ...
            <div class="space-y-2 sm:space-y-3">
                ...
                <div class="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-4">
                    <label
                        ...
                        title={$locales('downloadbuttons.update_existing_firmware')}
                    >
                        <input
                            type="radio"
                            bind:group={firmwareMode}
                            value="update"
                            class="text-orange-500 focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 focus:ring-offset-gray-900"
                        />
                        <span class="text-orange-100"
                            >{$locales('downloadbuttons.update_device')}</span
                        >
                        <span
                            class="max-w-[100px] text-xs break-words text-orange-400 sm:max-w-none sm:break-normal"
                            >{$locales('downloadbuttons.preserves_settings')}</span
                        >
                    </label>
                    <label
                        class="flex cursor-pointer items-center space-x-2"
                        title={$locales('downloadbuttons.complete_firmware_wipe')}
                    >
                        <input
                            type="radio"
                            bind:group={firmwareMode}
                        ...
                        <span
                            class="max-w-[100px] text-xs break-words text-orange-400 sm:max-w-none sm:break-normal"
                            >{$locales('downloadbuttons.wipe_reinstall')}</span
                        >
                    </label>
                </div>
            </div>
            ...
            {#each downloadOptions.slice(0, 2) as option}
                ...
                >
                    <span class="flex items-center justify-center">
                        <span class="mr-2 text-base">{option.icon}</span>
        ...
        {#if downloadOptions.length > 2}
            <div class="flex justify-center">
                ...
                >
                    {showMoreOptions
                        ? $locales('common.show_less')
                        : $locales('common.more_options')}
                </button>
            </div>

            {#if showMoreOptions}
                <div class="space-y-3 rounded-lg border-t border-orange-500 pt-4">
                    <h3 class="text-lg font-semibold text-orange-200">
                        {$locales('downloadbuttons.additional_download_options')}
                    </h3>
...
<dialog
    bind:this={espWebToolsDialog}
    class="backdrop:bg-opacity-50 rounded-lg border border-orange-600 bg-gray-800 p-6 backdrop:bg-black"
>
    <h3 class="mb-4 text-lg font-semibold text-orange-200">
        {$locales('downloadbuttons.esp_web_tools_title')}
    </h3>
    <div class="space-y-4">
        ...
        <div class="space-y-3">
            <h4 class="mb-2 font-medium text-orange-200">
                {$locales('downloadbuttons.instructions')}
            </h4>
            <ol class="list-inside list-decimal space-y-2 text-sm text-orange-300">
            ...
            <div class="max-h-60 space-y-2 overflow-y-auto">
                {#each [...availableArchives].sort( (a, b) => b.name.localeCompare(a.name) ) as archive}
                    ...
                    >
                        ...
                        <span class="ml-2 flex-shrink-0 text-xs text-orange-400"
                            >{formatFileSize(archive.size)}</span
                        >
                    </button>
                {/each}
            </div>
