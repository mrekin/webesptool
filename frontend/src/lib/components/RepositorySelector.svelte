<script lang="ts">
    ...
    let { onOpenStats = undefined }: Props = $props();

    // Get current repository description
    const currentRepoDesc = $derived(
        $availableSources.find((s: any) => s.src === $deviceSelection.source)?.desc || ''
    );
...
<div class="w-full space-y-2">
    <div class="flex w-full flex-wrap items-center gap-2">
        {#each $availableSources as source ((source as any).src)}
            <button
                type="button"
                onclick={() => handleSourceChange((source as any).src)}
                class="rounded-md px-3 py-1.5 text-sm transition-colors {$deviceSelection.source ===
                (source as any).src
        ...
        {/each}
        {#if $uiState.statsEnabled && $uiState.experimentalFeatures && onOpenStats}
            <button
                type="button"
                onclick={onOpenStats}
