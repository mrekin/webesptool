<script lang="ts">
    ...
    function handleKeydown(event: KeyboardEvent) {
        ...
        if (event.key === 'ArrowDown') {
            ...
            // Priority: autocomplete over history
            if (suggestion) {
                const prevSuggestion = getPreviousSuggestion(
                    value,
                    suggestion,
                    showCommandShortDescriptions
                );
...
<div class="command-input-container flex flex-1 items-center">
    ...
    <div class="input-wrapper relative flex-1">
        <input
            ...
            {placeholder}
            class="command-input relative w-full rounded-md border border-gray-600 bg-gray-700 py-2 pr-8 pl-3 text-sm text-white placeholder-gray-400 focus:border-orange-600 focus:ring-1 focus:ring-orange-600 focus:outline-none"
        ...
        {#if mode === 'meshcore'}
            <div
                class="meshcore-indicator absolute top-1/2 right-2 flex h-5 w-6 -translate-y-1/2 cursor-help items-center justify-center rounded bg-green-600/80 text-[0.625rem] font-bold tracking-tight text-white backdrop-blur-sm select-none"
                title="MeshCore mode"
            >
                MC
            </div>
        ...
        {#if suggestion}
            <div
                class="autocomplete-overlay pointer-events-none absolute inset-y-0 right-10 left-0 flex items-center overflow-hidden px-3"
            ...
            >
                ...
                <span class="whitespace-pre" style="visibility: hidden;">{value}</span><span
                    class="whitespace-pre text-gray-400">{suggestion.text}</span
                >
