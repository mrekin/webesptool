<script lang="ts">
    ...
    function showNewYearTree(): boolean {
        ...
        const now = new Date();
        console.log(
            '🎄 New Year Tree Check - Current date:',
            now.toISOString(),
            'Local:',
            now.toLocaleString()
        );
    ...
    <p class="text-center text-xs text-gray-400">
        ...
        {#if showCoffee}
            <a
                ...
                class="hover:animate-sparkle coffee-link ml-1 cursor-pointer border-none bg-transparent p-0 align-middle text-xs transition-all duration-300 hover:scale-125"
