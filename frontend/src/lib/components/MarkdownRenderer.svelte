    ...
    async function loadContent() {
        try {
            ...
            if (filename) {
                ...
                console.info(
                    `[MarkdownRenderer] Successfully loaded: ${filename} for locale: ${currentLocale}`
                );
                return;
            }
...
{#if unwrap}
    ...
    {#if content}
        <Markdown source={content} {renderer} breaks={true} linkify={true} class={className} />
    {:else if loading}
...
{:else}
    ...
    <div bind:this={markdownContainer} class="{wrapperClass} {className} markdown-container">
        ...
        {:else if error}
            <div class="bg-opacity-20 rounded-lg border border-red-600 bg-red-900 p-4">
                <h3 class="mb-2 font-semibold text-red-400">Error loading content</h3>
                <p class="text-red-300">{error}</p>
            </div>
        {:else if content}
            {#if hide}
                <!-- Spoiler mode -->
                <div class="ml-4 overflow-hidden rounded-lg border border-orange-600">
                    <button
                        onclick={() => (isSpoilerOpen = !isSpoilerOpen)}
                        class="flex w-full items-center justify-between bg-orange-900/30 p-3 text-left transition-all duration-300 hover:border-orange-500 hover:bg-orange-900/50"
                    ...
                    >
                        ...
                        <span
                            ...
                            style="transform: {isSpoilerOpen ? 'rotate(180deg)' : 'rotate(0deg)'}"
                        >
                            ▼
                        </span>
                    </button>
                    {#if isSpoilerOpen}
                        <div class="animate-fade-in border-t border-orange-600 p-4">
                            <Markdown source={content} {renderer} breaks={true} linkify={true} />
                        </div>
                    {/if}
                </div>
            {:else}
