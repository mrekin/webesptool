...
export function loadParsingRules(jsonContent: string | RawParsingRuleConfig): ParsingRule[] {
    try {
        ...
        return config.labels.map(
            (rawRule: RawParsingRule): ParsingRule => ({
                ...rawRule,
                        ...
                        for (const r of rule.rules ?? []) {
                            if (r.dropAfter) {
                                try {
                                    ...
                                    console.error(
                                        `Invalid dropAfter regex for token ${rule.name}:`,
                                        regexError
                                    );
                                }
                            }
                            ...
                            if (rule.regexp) {
                                try {
                                    ...
                                    if (match) {
                                        ...
                                        value =
                                            match.length > 1 ? (match[1] ?? match[0]) : match[0];
                                    }
                                } catch (regexError) {
                                    console.error(
                                        `Invalid regexp for token ${rule.name}:`,
                                        regexError
                                    );
                                }
