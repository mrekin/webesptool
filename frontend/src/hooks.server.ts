...
export async function handle({ event, resolve }) {
    ...
    const currentLocale = supportedLocales.includes(detectedLocale as any)
        ? detectedLocale
        : defaultLocale;
    ...
    let apiPath = null;
    // /api/geocode and /api/zones/* are served by SvelteKit routes (+server.ts),
    // not proxied to the Python backend — exclude them so resolve(event) reaches
    // those routes.
    if (
    ...
    const response = await resolve(event, {
        transformPageChunk: ({ html }) => {
            ...
            return html.replace('<body', `<body data-server-locale="${currentLocale}"`);
        }
    });
